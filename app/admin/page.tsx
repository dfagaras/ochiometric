...
export default async function AdminPage() {
  ...
  return <main className="admin-shell"><header className="admin-header"><Link href="/" className="public-brand" aria-label="Ochiometric — pagina principală"><Image className="brand-logo" src="/ochiometric-logo.png" width={869} height={209} alt="Ochiometric" priority /></Link><div><span>{user.displayName}</span><Link href={chatGPTSignOutPath("/")}>Ieșire</Link></div></header><div className="admin-intro"><span className="eyebrow">ATELIERUL DE ÎNTREBĂRI</span><h1>Planifică următoarea provocare.</h1><p>Creează, validează, programează și publică edițiile Ochiometric.</p></div><AdminClient initialPuzzles={puzzles} /></main>;
}
