...
type TodayPuzzle = { edition:number; publishDate:string; questions:Question[] };
type Reveal = { position:number; guess:number; answer:number; factor:number; explanation:string; sourceLabel:string; sourceUrl:string; napkinMath:string; completed:boolean; score:number|null };
...
function dateFmt(value?:string) { return value ? new Intl.DateTimeFormat("ro-RO", { day:"numeric", month:"long", year:"numeric", timeZone:"Europe/Bucharest" }).format(new Date(`${value}T12:00:00Z`)).toUpperCase() : "SE ÎNCARCĂ"; }
...
export default function Home() {
  ...
  const [q,setQ]=useState(0); const [input,setInput]=useState(""); const [puzzle,setPuzzle]=useState<TodayPuzzle|null>(null); const [questions,setQuestions]=useState<Question[]>([]); const [reveals,setReveals]=useState<Reveal[]>([]);
  ...
  useEffect(()=>{ const timer=setTimeout(()=>{ try { setHistory(JSON.parse(localStorage.getItem("din-ochi-history")||"[]")); } catch{} void fetch("/api/puzzles/today").then(async response=>{if(response.ok){const data=await response.json() as {puzzle:TodayPuzzle};setPuzzle(data.puzzle)}}); },0); return()=>clearTimeout(timer); },[]);
  ...
  async function submit(){ const value=parseGroupedInteger(input); if(!Number.isFinite(value)||value<=0||busy)return; setBusy(true);setGameError("");try{const response=await fetch("/api/attempts/answer",{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({position:q+1,guess:value})});const data=await response.json() as {reveal?:Reveal;error?:string};if(!response.ok||!data.reveal)throw new Error(data.error||"Estimarea nu a putut fi blocată.");setReveals([...reveals,data.reveal]);setRevealed(true)}catch(error){setGameError(error instanceof Error?error.message:"A apărut o eroare.")}finally{setBusy(false)} }
  function next(){ if(q<2){setQ(q+1);setInput("");setRevealed(false)} else { const no=puzzle?.edition??1; const game={no,date:puzzle?.publishDate??new Date().toISOString(),score:dailyScore,factors}; const next=[game,...history.filter(h=>h.no!==no)];setHistory(next);localStorage.setItem("din-ochi-history",JSON.stringify(next)); setView("result");void loadStatistics(); } }
  async function restart(){setQ(0);setInput("");setReveals([]);setQuestions([]);setStatistics(null);setRevealed(false);setGameError("");setView("play");setBusy(true);try{const attemptResponse=await fetch("/api/attempts",{method:"POST"});const attemptData=await attemptResponse.json() as {attempt?:{completedAt:string|null;answers:Array<Omit<Reveal,"completed"|"score">>};error?:string};if(!attemptResponse.ok||!attemptData.attempt)throw new Error(attemptData.error||"Jocul de azi nu este disponibil.");const response=await fetch("/api/puzzles/today");const data=await response.json() as {puzzle?:TodayPuzzle;error?:string};if(!response.ok||!data.puzzle)throw new Error(data.error||"Jocul nu a putut fi încărcat.");const saved=attemptData.attempt.answers.map(answer=>({...answer,completed:false,score:null}));setPuzzle(data.puzzle);setQuestions(data.puzzle.questions);setReveals(saved);if(attemptData.attempt.completedAt){setView("result");await loadStatistics()}else{setQ(saved.length);setRevealed(false)}}catch(error){setGameError(error instanceof Error?error.message:"A apărut o eroare.")}finally{setBusy(false)}}
  async function share(){const url="https://din-ochi.dragosfagaras.chatgpt.site"; const text=`Ochiometric #${String(puzzle?.edition??1).padStart(3,"0")} — ${scoreFmt(dailyScore)}${percentile?` · Top ${percentile}%`:""}\nMă bați?`; if(navigator.share) await navigator.share({title:"Ochiometric",text,url}); else await navigator.clipboard.writeText(`${text}\n${url}`);}
  ...
  return <main className={`app view-${view} ${view==="play"?`question-${q+1}`:""}`}>
    ...
    {view==="home"&&<><Header onMenu={()=>setMenu(true)} onBack={()=>{}}/><section className="hero card">
      <div className="edition">OCHIOMETRIC · NR. {String(puzzle?.edition??1).padStart(3,"0")}</div><h1>Cât de bine<br/><em>estimezi?</em></h1><p>Trei întrebări. Fără Google. Doar logică, instinct și puțină matematică.</p>
      <div className="dots"><b>1</b><b>2</b><b>3</b></div><button className="primary" onClick={restart}>JOACĂ AZI <span>→</span></button><small>{dateFmt(puzzle?.publishDate)} · DUREAZĂ 3 MINUTE</small>
    ...
    {view==="play"&&<><Header onMenu={()=>setMenu(true)} onBack={()=>setView("home")} back/><section className="play-card card">{gameError&&<p role="alert">{gameError}</p>}{!questions[q]?<p>{busy?"Se încarcă jocul de azi…":"Jocul nu este disponibil."}</p>:<><div className="progress"><span>ÎNTREBAREA {q+1} DIN 3</span><div><i style={{width:`${((q+(revealed?1:0))/3)*100}%`}}/></div></div><h2>{questions[q].prompt}</h2>{!revealed?<><label htmlFor="guess">ESTIMAREA TA</label><div className="guess"><input id="guess" className={guessSizeClass(input)} inputMode="numeric" autoComplete="off" value={input} onChange={e=>setInput(formatGroupedInteger(e.target.value))} onKeyDown={e=>e.key==="Enter"&&submit()} placeholder="0" autoFocus/><span>{questions[q].unit}</span></div><p className="micro">Nu căuta. Ai încredere în instinct.</p><button className="primary" disabled={busy} onClick={submit}>{busy?"SE BLOCHEAZĂ…":"BLOCHEAZĂ RĂSPUNSUL"}</button></>:<div className="reveal"><div className="compare"><article><small>AI SPUS</small><b>{fmt(reveals[q].guess)}</b></article><article><small>RĂSPUNS</small><b>{fmt(reveals[q].answer)}</b></article></div><div className="factor"><b>{scoreFmt(reveals[q].factor)}</b><span>{reveals[q].guess>reveals[q].answer?"PREA MULT ↑":"PREA PUȚIN ↓"}</span></div><p className="answer-note">{reveals[q].explanation}</p>{reveals[q].napkinMath&&<CalculationGuide value={reveals[q].napkinMath} sourceLabel={reveals[q].sourceLabel} sourceUrl={reveals[q].sourceUrl}/>}<button className="primary" onClick={next}>{q<2?"URMĂTOAREA ÎNTREBARE":"VEZI SCORUL"} →</button></div>}</>}</section></>}
