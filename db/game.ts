...
export async function publicPuzzleByEdition(db:GameDatabase,edition:number){
  ...
  const {results}=await db.prepare(`SELECT p.edition,p.publish_date,q.position,q.prompt,q.unit FROM puzzles p JOIN questions q ON q.puzzle_id=p.id WHERE p.edition=? AND p.status IN ('published','archived') ORDER BY q.position`).bind(edition).all<{edition:number;publish_date:string;position:number;prompt:string;unit:string}>();
  if(!results.length)return null;
  return {edition:results[0].edition,publishDate:results[0].publish_date,questions:results.map(({position,prompt,unit})=>({position,prompt,unit}))};
}
...
export async function lockAnswerByEdition(db:GameDatabase,playerId:string,edition:number,position:number,guess:number){
...
async function lockAnswerForPuzzle(db:GameDatabase,playerId:string,selector:"p.publish_date"|"p.edition",value:string|number,position:number,guess:number){
  ...
  const playableStatus=selector==="p.edition"?"p.status IN ('published','archived')":"p.status='published'";
  const row=await db.prepare(`SELECT a.id attempt_id,q.id question_id,q.answer,q.explanation FROM attempts a JOIN puzzles p ON p.id=a.puzzle_id JOIN questions q ON q.puzzle_id=p.id AND q.position=? WHERE a.player_id=? AND ${selector}=? AND ${playableStatus} AND a.completed_at IS NULL`).bind(position,playerId,value).first<{attempt_id:number;question_id:number;answer:number;explanation:string}>();
  ...
  if(completed)await db.prepare(`UPDATE attempts SET completed_at=CURRENT_TIMESTAMP,score=?,public_id=? WHERE id=? AND completed_at IS NULL`).bind(aggregate.score,publicResultId,row.attempt_id).run();
  return {position,guess,answer:row.answer,factor,explanation:row.explanation,completed,score:completed?aggregate!.score:null,publicResultId};
}
